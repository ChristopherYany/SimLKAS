...
- [5. SimLKAS LKS Fusion EKF](#5-simlkas-lks-fusion-ekf)
  - [5.1. Data Flow and Interfaces](#51-data-flow-and-interfaces)
  - [5.2. State, Dynamics, and Measurements](#52-state-dynamics-and-measurements)
  - [5.3. Adaptive Covariance and Smoothing](#53-adaptive-covariance-and-smoothing)
  - [5.4. Key Parameters](#54-key-parameters)
  - [5.5. Minimal Verification](#55-minimal-verification)
...
## 5. SimLKAS LKS Fusion EKF

This repository also includes a lightweight EKF tuned for lane keeping in SimLKAS. The implementation
is in `modules/fusion/lks_fusion.py` and fuses:

- Vision lane parameters from perception (`LegacyLaneDetector` or LaneNet)
- IMU (gyro + accel)
- GNSS (speed from lat/lon deltas)

The output remains compatible with the existing controller by emitting `lane_params` in the same
`{"left": (vx, vy, x0, y0), "right": (...)}` structure.

### 5.1. Data Flow and Interfaces

Interfaces exposed by `LaneFusionEKF`:

- `push_imu(t, accel, gyro)`
- `push_gnss(t, lat, lon, alt)`
- `push_lane(t, lane_params, target_speed_kph)`
- `advance(t_target)`
- `get_lane_params()`

The simulator loop should:
1) push incoming IMU/GNSS samples as they arrive,
2) push the current frame's `lane_params` with its timestamp,
3) call `advance(sim_time)` once per tick, and
4) use `get_lane_params()` for control.

### 5.2. State, Dynamics, and Measurements

State vector:

```
x = [d, psi, v]
d   : lateral offset in meters (positive when lane center is to the right)
psi : heading error in radians (lane direction minus vehicle heading)
...
```
d_dot   = v * sin(psi)
psi_dot = yaw_rate_sign * gyro_z
v_dot   = accel_x_sign * accel_x
```

Vision measurement from `lane_params` at a fixed lookahead y:

```
d_meas  = (center_x - lane_center_px) * (lane_width_m / lane_width_px)
psi_meas = lane_angle - pi/2
```

GNSS provides speed updates from successive lat/lon samples converted to local meters.

### 5.3. Adaptive Covariance and Smoothing

Lane measurement confidence `c` is computed from:

- availability (both lines vs single line)
- lane width consistency (right_x - left_x close to expected width)
- left/right angle agreement

Confidence is smoothed with an exponential filter, and a hysteresis gate (`conf_low_th`,
`conf_high_th`) prevents toggling. The lane measurement covariance is scaled as:

```
R_lane = R_base * (R_min + (1 - c_smooth) * (R_max - R_min))
```

If the innovation fails the NIS gate (`nis_gate`), `R_lane` is inflated to `R_max`. Residual
clipping and an output low-pass (`output_tau`) prevent sudden jumps when vision quality drops
or recovers.

### 5.4. Key Parameters

The main knobs live in `FusionConfig`:

- `lane_width_px`, `lane_width_m`, `lane_center_px`
- `lookahead_px_base`, `lookahead_px_speed_gain`
- `lane_d_noise`, `lane_psi_noise`
- `lane_r_scale_min`, `lane_r_scale_max`
- `conf_low_th`, `conf_high_th`, `conf_alpha`
- `output_tau`, `residual_clip_d`, `residual_clip_psi`
- `imu_accel_noise`, `imu_gyro_noise`, `gnss_speed_noise`
- `yaw_rate_sign`, `accel_x_sign` (coordinate sign fixes)

### 5.5. Minimal Verification

1) Enable fusion in the config (see `lkas.fusion.enabled` and `lkas.fusion.enable_adaptive`).
2) Run the same scenario twice: once with `enable_adaptive=true`, once with `false`.
3) Compare `results/*_trajectory.csv` lateral deviation or visually inspect overlay stability.
